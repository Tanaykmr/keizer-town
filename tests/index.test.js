...
const axiosHandler = async (method, url, data, config = {}) => {
	try {
		// Add `validateStatus` to the config
		const finalConfig = {
			...config,
			validateStatus: (status) => status < 500, // Resolve for any status < 500
		};

		// Dynamically call the axios method
		const res =
			method === "get" || method === "delete"
				? await axios2[method](url, finalConfig) // GET/DELETE only need `url` and `config`
				: await axios2[method](url, data, finalConfig); // POST/PUT need `url`, `data`, and `config`
...
const axios = {
	post: (url, data, config) => axiosHandler("post", url, data, config),
	get: (url, config) => axiosHandler("get", url, null, config),
	put: (url, data, config) => axiosHandler("put", url, data, config),
	delete: (url, config) => axiosHandler("delete", url, null, config),
};
...
describe("User metadata endpoint", () => {
	...
	beforeAll(async () => {
		let username = `kirat-${Math.random()}`;
		const password = "123456";
		...
		// console.log("the adminToken is : ", adminToken);
...
describe("Space information", () => {
	...
	test("Admin has no spaces initially", async () => {
		...
		// console.log("The admin's spaces are: ", response.data.spaces);
